...
def compare_records(azara_record, rui_record):
    ...
    if rui_record[1] == azara_coordinates:
        return True
    return False
...
def create_record(azara_record, rui_record):
    ...
    comparisson = compare_records(azara_record,rui_record)
    if comparisson:
...
def clean_up(combined_record_group):
    ...
    """
    new_lst = list(combined_record_group)
    big_str = """"""
    for combined_record in new_lst:
        new_rec = list(combined_record)
        new_rec.remove(new_rec[1])
        big_str+= str(tuple(new_rec))+ '\n'
    return big_str